...
module_predoSimpleDoppler::module_predoSimpleDoppler()
{
...
void module_predoSimpleDoppler::print_csi_packet(csi_packet *packet)
{
...
bool module_predoSimpleDoppler::read_bfee(csi_packet *packet, void *buffer)
{
    data = (uint8_t*)buffer;
...
bool module_predoSimpleDoppler::read_bf_file(const char* filename, csi_packet* &packet, int32_t *count)
{
    long len, num;
...
bool module_predoSimpleDoppler::Module_Scaled(csi_packet *packet, const int count)
{
...
bool module_predoSimpleDoppler::get_scaled_csi(csi_packet *packet)
{
...
double module_predoSimpleDoppler::get_total_rss(csi_packet *packet)
{
    double rssi_mag = 0.0;
    if (packet->rssi_a != 0) {
...
double module_predoSimpleDoppler::get_total_csi(csi_packet *packet)
{
...
bool module_predoSimpleDoppler::Module_predoSimpleDoppler_Shm(csi_packet *packet, void *buffer)
{
    char *data = (char*)buffer;
    return read_bfee(packet, data+3);
}

bool module_predoSimpleDoppler::Module_predoSimpleDoppler_File(const char* filename, csi_packet* &packet, const int downsampling, int *count)
{
    if (!read_bf_file(filename, packet, count)) {
        return false;
    }
